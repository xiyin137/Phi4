...
/-- Linear-growth frontier from explicit WP1-style polynomial L² control of
    shifted cutoff deviations plus per-exponent uniform shifted-cutoff
    exponential integral bounds.
    This route constructs `InteractionWeightModel` directly from assumption-
    explicit hypotheses and then applies `gap_phi4_linear_growth`. -/
theorem gap_phi4_linear_growth_of_sq_moment_polynomial_bound_and_uniform_integral_bound
    (params : Phi4Params)
    [SchwingerLimitModel params]
    [OSAxiomCoreModel params]
    [OSDistributionE2Model params]
    [OSE4ClusterModel params]
    (C β : ℝ) (hC : 0 ≤ C) (hβmom : 1 < β)
    (hInt :
      ∀ (Λ : Rectangle) (n : ℕ),
        Integrable
          (fun ω : FieldConfig2D =>
  ...
  have hbound' :
    ...
    intro Λ p hp
    exact hbound Λ p hp
  rcases interactionWeightModel_nonempty_of_sq_moment_polynomial_bound_and_uniform_integral_bound
      (params := params) (C := C) (β := β) hC hβmom hInt hM
      (hcutoff_meas := fun Λ n => by
        simpa using hcutoff_meas Λ (standardUVCutoffSeq (n + 1)))
      (hbound := hbound') with ⟨hW⟩
  letI : InteractionWeightModel params := hW
  exact gap_phi4_linear_growth params hsmall alpha beta gamma hbeta
    huniform hcompat hreduce hdense

/-- Linear-growth frontier from explicit WP1-style polynomial L² control of
    shifted cutoff deviations plus per-exponent geometric shifted-cutoff
    exponential integral bounds.
...
/-- Linear-growth frontier from:
    1) polynomial-decay squared-moment cutoff-to-limit control, and
    2) per-exponent geometric shifted-cutoff real-parameter exponential-moment
       bounds for each `q > 0`.
    This keeps the WP1 input faithful to Glimm-Jaffe 8.6.2 and avoids
    geometric assumptions at `p = 0`. -/
theorem gap_phi4_linear_growth_of_sq_moment_polynomial_bound_and_geometric_exp_moment_bound
    (params : Phi4Params)
    [SchwingerLimitModel params]
    [OSAxiomCoreModel params]
    [OSDistributionE2Model params]
    ...
    ∃ OS : OsterwalderSchraderAxioms 1,
      OS.S = phi4SchwingerFunctions params ∧
      Nonempty (OSLinearGrowthCondition 1 OS) := by
  rcases interactionWeightModel_nonempty_of_sq_moment_polynomial_bound_and_geometric_exp_moment_bound
      (params := params) (C := C) (β := β) hC hβmom hInt hM
      (hcutoff_meas := fun Λ n => by
        simpa using hcutoff_meas Λ (standardUVCutoffSeq (n + 1)))
      hgeom with ⟨hW⟩
...
/-- Linear-growth frontier with assumption-explicit real-parameterized UV
    convergence and per-exponent geometric shifted-cutoff real-parameter
    exponential-moment bounds:
    `∫ exp(-(q * interactionCutoff(κ_{n+1}))) ≤ D * r^n` for each `q > 0`.
    This matches the natural Theorem 8.6.2 input shape and avoids requiring
    geometric assumptions at `p = 0`. -/
theorem gap_phi4_linear_growth_of_tendsto_ae_and_geometric_exp_moment_bound
    (params : Phi4Params)
    [SchwingerLimitModel params]
    [OSAxiomCoreModel params]
    [OSDistributionE2Model params]
    ...
    ∃ OS : OsterwalderSchraderAxioms 1,
      OS.S = phi4SchwingerFunctions params ∧
      Nonempty (OSLinearGrowthCondition 1 OS) := by
  rcases interactionWeightModel_nonempty_of_tendsto_ae_and_geometric_exp_moment_bound
      (params := params) hcutoff_tendsto_ae
      (hcutoff_meas := hcutoff_meas) hgeom with ⟨hW⟩
  letI : InteractionWeightModel params := hW
  exact gap_phi4_linear_growth params hsmall alpha beta gamma hbeta
    huniform hcompat hreduce hdense
